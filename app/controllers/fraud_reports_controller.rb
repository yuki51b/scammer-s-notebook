class FraudReportsController < ApplicationController
  ...
  def fraud_report_params
    params.require(:fraud_report).permit(:contact_method, :contact_content, :information, :urgent_action,
                                         :payment_method, :company_info, :who_person)
  end
  ...
  def scam_name_general_prompt(fraud_report_params)
    ...
    <<~PROMPT
      ...
      ## 制約条件
      - 出力の際は"出力の説明"と"例"を十分に理解して、"出力フォーマット"を必ず守って出力してください。
  ...
  def handle_scam_diagnosis(response_name)
    ...
    create_scam = Scam.new(name: response_name.strip, content: scam_content, point_1: scam_point_1,
                           point_2: scam_point_2, point_3: scam_point_3, scam_strategy:)
    return unless create_scam.save
